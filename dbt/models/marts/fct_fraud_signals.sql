...
--
-- Note: the sample is synthetic/random, so a raised flag is coincidental. This is
...
round_dep as (
    select
        player_id,
        countif(transaction_type = 'deposit' and mod(amount, 100) = 0) as n_round
...
reg_p90 as (
    select approx_quantiles(n, 100)[offset(90)] as p90 from reg_by_day
),

reg_burst as (
    select p.player_id
    from {{ ref('stg_players') }} as p
    inner join reg_by_day as d on p.created_at = d.d
...
scored as (
    select
        ...
        coalesce(
            f.total_deposits > 0 and safe_divide(f.total_bets, f.total_deposits) < 0.2
            and f.total_withdrawals > 0, false
        ) as s_aml_low_play,
    ...
    left join act as a using (player_id)
    left join round_dep as r using (player_id)
),
    ...
    select
        *,
        (
            ...
            + cast(s_ledger_anomaly as int64) + cast(s_net_negative as int64)
        ) as risk_score
    from scored
)

-- risk_tier: categorical label bucketed in Gold (not DAX) so it is a first-class,
-- sliceable attribute of the row and stays a single source of truth.
select
    *,
    case
